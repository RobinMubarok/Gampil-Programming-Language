...
static GampilType parse_type_spec(Parser* p, int* is_pointer, int* array_sizes, int* num_dims);
...
static AstNode* parse_primary(Parser* p) {
    ...
    /* Removed Address-of: moved to parse_unary */
    ...
    if (is_type_tok(p->current.type)) {
        int is_ptr = 0; int arr_szs[4]; int n_dims = 0;
        GampilType gt = parse_type_spec(p, &is_ptr, arr_szs, &n_dims);
    ...
    /* Identifiers: may be followed by '[' (call) or '(' (index) or '.' (field) */
    if (check(p, TOK_IDENT) || check(p, TOK_PRINTF) || check(p, TOK_PRINTN)
        || check(p, TOK_PRINT)) {
        
        AstKind base_call_kind = AST_CALL_EXPR;
        if (check(p, TOK_PRINTF)) base_call_kind = AST_PRINTF_CALL;
        else if (check(p, TOK_PRINTN)) base_call_kind = AST_PRINTN_CALL;
        else if (check(p, TOK_PRINT))  base_call_kind = AST_PRINT_CALL;
        ...
        /* Unified trailing accessors loop: chained .field or (index) or [call] */
        while (check(p, TOK_DOT) || check(p, TOK_LPAREN) || check(p, TOK_LBRACKET)) {
            if (check(p, TOK_DOT)) {
                advance(p);
                AstNode* fa = ast_new(AST_FIELD_EXPR, p->current.line, p->current.col);
                ...
                advance(p); /* ( */
                AstNode* idx = ast_new(AST_INDEX_EXPR, p->current.line, p->current.col);
                ...
                if (n->kind == AST_IDENT && base_call_kind != AST_CALL_EXPR) {
                    call_kind = base_call_kind;
                }
                AstNode* call_node = ast_new(call_kind, p->current.line, p->current.col);
                call_node->as.call.callee = n;
                ...
                while (!check(p, TOK_RBRACKET) && !check(p, TOK_EOF)) {
                    ...
                    if (check(p, TOK_IDENT) && check2(p, TOK_BE)) {
                        ...
                        advance(p); advance(p);
                        AstNode* karg = ast_new(AST_ASSIGN_STMT, p->current.line, p->current.col);
                        AstNode* target_id = ast_new(AST_IDENT, p->current.line, p->current.col);
                        target_id->as.ident.name = kname;
                        karg->as.assign.target_expr = target_id;
                ...
                if (call_node->kind == AST_CALL_EXPR && call_node->as.call.callee->kind == AST_IDENT) {
                    char* cname = call_node->as.call.callee->as.ident.name;
                    if (is_python_cast_name(cname)) {
                        call_node->kind = AST_PYCAST_EXPR;
                        call_node->as.py_cast.py_type = strdup(cname);
                        call_node->as.py_cast.expr = args ? args->node : NULL;
                        ast_free(call_node->as.call.callee);
                        if (args) { /* Only free the list shell, nodes are preserved or leaked slightly */ free(args); }
                    } else if (cname[0] == 'z') {
                        GampilType t = str_to_gtype(cname + 1);
                        if (t != GTYPE_UNKNOWN) {
                            call_node->kind = AST_GACAST_EXPR;
                            call_node->as.py_cast.target_type = t;
                            call_node->as.py_cast.expr = args ? args->node : NULL;
                            ast_free(call_node->as.call.callee);
                            if (args) { free(args); }
                        }
...
static AstNode* parse_unary(Parser* p) {
    ...
    if (t == TOK_AT) {
        ...
        n->as.addr_of.target = parse_unary(p);
        return n;
    }
    ...
    *num_dims = 0;
    ...
    while (check(p, TOK_LPAREN)) {
        ...
        if (check(p, TOK_RPAREN)) {
            ...
            if (check(p, TOK_INT_LIT)) {
                if (*num_dims < 4) {
                    array_sizes[*num_dims] = (int)strtol(p->current.value, NULL, 10);
                    (*num_dims)++;
                }
                advance(p);
            }
...
static AstNode* parse_var_decl_tail(Parser* p, GampilType gt, int is_ptr, int* arr_szs, int n_dims, char* name, int ln, int col) {
    AstNode* n = ast_new(AST_VAR_DECL, ln, col);
    n->as.var_decl.type       = gt;
    n->as.var_decl.name       = name;
    n->as.var_decl.is_pointer = is_ptr;
    n->as.var_decl.num_dims   = n_dims;
    for (int i=0; i<n_dims; i++) n->as.var_decl.array_sizes[i] = arr_szs[i];
...
static AstNode* parse_redo_loop(Parser* p) {
    ...
    while (1) {
        ...
        if (is_type_tok(p->current.type)) {
            gt = parse_type_spec(p, &is_ptr, arr_szs, &n_dims);
        ...
        iter_node->as.var_decl.num_dims = n_dims;
        for(int i=0; i<n_dims; i++) iter_node->as.var_decl.array_sizes[i] = arr_szs[i];
...
static AstNode* parse_func_decl_tail(Parser* p, GampilType ret, int is_ptr, int* arr_szs, int n_dims, char* name, int ln, int col) {
    (void)arr_szs; (void)n_dims; /* functions don't return static arrays in Gampil currently */
...
static AstNode* parse_stmt(Parser* p) {
    ...
    if (is_type_tok(p->current.type)) {
        ...
        if (check(p, TOK_FIELD) && check2(p, TOK_LPAREN)) {
            ...
            if (check(p, TOK_LBRACE)) {
                ...
                while (!check(p, TOK_RBRACE) && !check(p, TOK_EOF)) {
                    if (is_type_tok(p->current.type)) {
                        int p_is_ptr = 0; int p_arr_szs[4]; int p_n_dims = 0;
                        GampilType pt = parse_type_spec(p, &p_is_ptr, p_arr_szs, &p_n_dims);
                        AstNode* fn = ast_new(AST_VAR_DECL, p->current.line, p->current.col);
                        fn->as.var_decl.type = pt;
                        fn->as.var_decl.is_pointer = p_is_ptr;
                        fn->as.var_decl.num_dims = p_n_dims;
                        for(int i=0; i<p_n_dims; i++) fn->as.var_decl.array_sizes[i] = p_arr_szs[i];
        ...
        int is_ptr2 = 0; int arr_szs2[4]; int n_dims2 = 0;
        GampilType gt2 = parse_type_spec(p, &is_ptr2, arr_szs2, &n_dims2);
        ...
        if (check(p, TOK_LBRACKET)) {
            if (reg_name) free(reg_name);
            return parse_func_decl_tail(p, gt2, is_ptr2, arr_szs2, n_dims2, name, ln_decl, col_decl);
        } else {
            ...
            t1->as.var_decl.num_dims = n_dims2;
            for(int i=0; i<n_dims2; i++) t1->as.var_decl.array_sizes[i] = arr_szs2[i];
            ...
            while (check(p, TOK_COMMA)) {
                advance(p);
                if (is_type_tok(p->current.type)) {
                    int p_ptr = 0; int p_arr_szs[4]; int p_n_dims = 0;
                    GampilType pt = parse_type_spec(p, &p_ptr, p_arr_szs, &p_n_dims);
                    ...
                    tn->as.var_decl.num_dims = p_n_dims;
                    for(int i=0; i<p_n_dims; i++) tn->as.var_decl.array_sizes[i] = p_arr_szs[i];
    ...
    /* Not a keyword statement: must be assignment or expression */
    AstNode* expr = parse_expr(p);
    if (!expr) return NULL;
    
    TokenType ct = p->current.type;
    
    /* Check for multi-assignment (let a, b be 1, 2) */
    if (ct == TOK_COMMA) {
        AstList* targets = NULL;
        targets = astlist_append(targets, expr);
        ...
        while (check(p, TOK_COMMA)) {
            ...
            if (is_type_tok(p->current.type)) {
                ...
                targets = astlist_append(targets, parse_expr(p));
            }
        }
        ...
        AstNode* m = ast_new(AST_MULTI_ASSIGN, expr->line, expr->col);
        m->as.multi_assign.targets = targets;
        m->as.multi_assign.values = values;
        return m;
    }
    ...
    if (ct == TOK_BE || ct == TOK_PLUS_BE  || ct == TOK_MINUS_BE ||
        ...
        advance(p);
        AstNode* n = ast_new(AST_ASSIGN_STMT, expr->line, expr->col);
        n->as.assign.target_expr = expr;
        n->as.assign.op = ct;
    ...
    AstNode* es = ast_new(AST_EXPR_STMT, expr->line, expr->col);
